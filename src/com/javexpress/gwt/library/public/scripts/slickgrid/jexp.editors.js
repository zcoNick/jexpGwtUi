...
function JexpDecimalCellEditor(args) {
	...
	this.validate = function() {
		return {
			valid: !isNaN(parseFloat($input.autoNumeric('get'))) || ($input.val()==''&&!args.column.required),
			msg: null
		};
	};
...
function JexpMaskEditor(args) {
	...
	this.validate = function() {
		return {
			valid: ((!$input.val() && !args.column.required) || $input.val().length>0),
			msg : null
		};
	};
...
function JexpDateEditor(args) {
	...
	this.validate = function() {
		return {
			valid: ((!$input.val() && !args.column.required) || $input.val().length==10),
			msg : null
		};
	};
...
function JexpLongTextEditor(args) {
	...
	this.validate = function() {
		return {
			valid: !args.column.required||($input.val()==''&&!args.column.required)||($input.val()!=''&&args.column.required),
			msg : null
		};
	};
...
function JexpSelectCellEditor(args) {
    ...
    this.validate = function() {
        return {
			valid: $select.val() || !args.column.required,
            msg: null
        };
    };
...
function JexpAutoCompleteEditor(args) {
	...
	this.init = function() {
		args.column.editorOptions.id = args.column.field + "_menu";
		...
		$div = $("<div class='input-group jexpEditGridEditorCell'><span class='input-group-addon'><i class='jexpHandCursor fa fa-search'></i></span><div>");
	...
	this.validate = function() {
		var isvalid = (lastItem==null && !args.column.required) || (lastItem.id || !args.column.required);
		return {
			valid : isvalid,
			msg : null
		};
